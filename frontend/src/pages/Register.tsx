  ...
  return (
    <div className="w-4/5 h-full flex flex-col justify-around items-center">
      ...
      <div className="grid gap-10 w-2/5 text-2xl *:flex *:flex-col [&_input]:rounded-full [&_input]:bg-[#7dcdff] [&_input]:h-10 [&_input]:px-3.5 [&_input]:outline-none [&_input]:border-none">
        <AuthInput id={"username"} icon={MdPersonOutline} type="text" label="Name" />
        ...
        <AuthInput id={"cpass"} icon={BsShieldLock} type="password" label="Confirm Password" />
      </div>
      <p className="text-xl">Already have an account? <Link className="text-blue-700 underline" to="/login">Login</Link></p>
      <Button value="Submit" onClick={() => { console.log("Register submitted") }} />
    </div>
  )
}
